...
Pure Python Native Zero-Dependency Regional Server
"""
...
def geocode_location_string(city_name):
    clean_query = city_name.strip().lower()
    if clean_query in MOCK_SOCIO_DATABASE:
        return MOCK_SOCIO_DATABASE[clean_query]
    for k, v in MOCK_SOCIO_DATABASE.items():
        if clean_query in k or clean_query in v['ward_name'].lower():
            return v
    return MOCK_SOCIO_DATABASE["haldwani"]


def apply_adaptive_monsoon_filter(t_celsius, rh, wind_speed):
    conditioned_rh = rh
    if 30.0 <= t_celsius <= 35.0 and rh > 80.0:
        if wind_speed > 2.0:
            dampening_factor = min(15.0, (wind_speed * 2.5))
            conditioned_rh = rh - dampening_factor
    return max(10.0, min(100.0, round(conditioned_rh, 1)))
...
def calculate_imd_heat_index(t_celsius, rel_h):
    ...
    t = (t_celsius * 9 / 5) + 32
    hi_simple = 0.5 * (t + 61.0 + ((t - 68.0) * 1.2) + (rel_h * 0.094))
    if hi_simple < 80:
    ...
    if (rel_h < 13) and (t >= 80 and t <= 112):
        h_i -= ((13 - rel_h) / 4) * math.sqrt(max(0.0, (17 - abs(t - 95.0)) / 17))
...
def calculate_swbgt(t_celsius, rel_h):
    e = (rel_h / 100.0) * 6.105 * math.exp((17.27 * t_celsius) / (237.7 + t_celsius))
    return round((0.567 * t_celsius) + (0.393 * e) + 3.94, 1)
...
def compute_mortality_risk(actual_temp, metrics, ward_meta):
    ...
    # COPIED EXACTLY FROM THE SCIKIT-LEARN REGRESSION ALIGNMENT:
    W_HEAT_INDEX = 0.0420
    W_WET_BULB   = 0.0650
    W_UTCI       = 0.0240

    # Balanced socio-demographic scaling factors
    W_ELDERLY    = 1.1500
    W_DENSITY    = 0.00003
    B_INTERCEPT  = -1.5200
...
def fetch_live_and_forecast_weather(lat, lon):
    """
    Connects to Open-Meteo REST API using native urllib connections.
    Uses an explicit range sequence to guarantee exactly 12 hours print.
    """
    url = f"https://api.open-meteo.com/v1/forecast?latitude={float(lat):.2f}&longitude={float(lon):.2f}&current=temperature_2m,relative_humidity_2m,wind_speed_10m&hourly=temperature_2m,relative_humidity_2m,wind_speed_10m&wind_speed_unit=ms"
...
def build_analysis_payload(city: str) -> dict:
    """Constructs the canonical output payload."""
    ...
    output_payload = {
        ...
        "iot_sensor_validation_gate": {
            "sensor_node_hardware_id": f"NODE-{ward_meta['ward_id']}",
...
class GovTechNetworkAPIHandler(BaseHTTPRequestHandler):
    """
    Pure Python native request socket listener.
    Maintains 100% zero-dependency architecture by bypassing external web frameworks entirely.
    """
    def do_GET(self):
        parsed_url = urllib.parse.urlparse(self.path)

        if parsed_url.path in ["/analyze", "/api/analyze"]:
            query_params = urllib.parse.parse_qs(parsed_url.query)
            city_list = query_params.get("city", ["haldwani"])
            city = city_list[0].strip() or "haldwani"

            output_payload = build_analysis_payload(city)
            ...
            if os.path.exists(index_path):
                ...
                self.send_error_payload("Index template not found", status=404)

        elif parsed_url.path.startswith("/static/"):
            rel_path = parsed_url.path[len("/static/"):].lstrip("/")
            ...
            if os.path.exists(file_path) and os.path.isfile(file_path):
                ...
                self.send_error_payload("Static file not found", status=404)
...
if __name__ == "__main__":
    ...
    print("      ACTIVE ZERO-DEPENDENCY REGIONAL SERVER LISTENING ON PORT 8000")
    print("      QUERY PATH LINK EXAMPLE -> http://127.0.0.1:8000/analyze?city=patna")
    try:
        httpd.serve_forever()
